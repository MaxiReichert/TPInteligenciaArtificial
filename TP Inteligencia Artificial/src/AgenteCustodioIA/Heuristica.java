...
public class Heuristica implements IEstimatedCostFunction {
	...
	public double getEstimatedCost(NTree node) {
		double distanciaALaMeta=0.0;
		double distanceTime=10;
		int indexCiudadanoMasCerca=0;
		...
		double xAgente = coordenadas[nodoPosAgente][0];
		double yAgente = coordenadas[nodoPosAgente][1];
		
		ArrayList<Ciudadano> ciudadanosFugados = ((AgenteCustodioState)node.getAgentState()).getCiudadanosFugados();
		
		while(!ciudadanosFugados.isEmpty()) {
			for(int i=0; i<ciudadanosFugados.size(); i++) {
				double xFugado= coordenadas[ciudadanosFugados.get(i).getPosicionActual()][0];
				double yFugado= coordenadas[ciudadanosFugados.get(i).getPosicionActual()][1];
				if(Math.sqrt(Math.pow(xFugado-xAgente, 2)+Math.pow(yFugado-yAgente, 2))<distanceTime) {
					distanceTime=Math.sqrt(Math.pow(xFugado-xAgente, 2)+Math.pow(yFugado-yAgente, 2));
					System.out.println(distanceTime);
					nodoPosAgente=ciudadanosFugados.get(i).getPosicionActual();
					indexCiudadanoMasCerca=i;
				}
			
				distanciaALaMeta=distanciaALaMeta+distanceTime;
				ciudadanosFugados.remove(indexCiudadanoMasCerca);
				distanceTime=10;
				indexCiudadanoMasCerca=0;
			}
		}
		
		return distanciaALaMeta;
	}
